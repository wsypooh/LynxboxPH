...
| **Document storage** | 20 files / 100MB | 200 files / 1GB | 1,000 files / 5GB | Unlimited (fair use) |
...
| Document storage | The document-upload/presign handler | `DocumentRepository.listByOwner(accountId)` (already exists) — count rows where `!deletedAt`, compare to `maxDocuments` (capped by **count**, not byte size, as a deliberate simplification — S3 object size isn't tracked on the `Document` record today; revisit if storage cost becomes a real problem) |
