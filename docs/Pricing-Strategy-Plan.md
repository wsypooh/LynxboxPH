...
17. **Considered adding a `'draft'` status (listing prepared but not yet public) and questioned whether `'sold'`/`'maintenance'` earn their keep — left both as-is, noted for later.** On `draft`: today a brand-new listing is public and counts against `maxProperties` the instant it's created, with no way to stage one privately first; worth adding, but needs a decision on whether drafts also count against the cap (probably yes, or it's a free way to stage unlimited hidden listings) and it wasn't built this pass. On `sold`/`maintenance`: confirmed they're functionally identical to each other and to any non-`available` status today — nothing distinguishes them beyond `status !== 'available'` hiding the listing from public search. They're being kept as-is since they cost nothing (plain enum values, no extra logic) and preserve real information for the owner's own record-keeping (why a listing is off-market), not because they're technically load-bearing.
