...
- **Upload method — presigned URL, direct-to-S3.** Verified by grep that `PropertyForm.tsx` (the only *live* property-image upload path) sends images as base64 in the JSON body through the Lambda (`PropertyForm.tsx:260-302`). A presigned-URL direct-to-S3 method already exists in the code (`S3Service.getPresignedUploadUrl`, `s3.ts:237-257`; `propertyService.uploadPropertyImage`, `propertyService.ts:422`) but has **zero callers** — it's unused scaffolding. Documents will use presigned URLs direct-to-S3 (better for larger PDFs, avoids Lambda/API Gateway payload limits), generalizing that existing scaffold rather than copying the base64 pattern.
